...
#include <odimm/rbtree.hpp>
#include <odimm/ip/address.hpp>
#include <odimm/ip/prefix.hpp>
#include <odimm/ll/mac_address.hpp>
#include <vector>
#include <map>
...
public:
	mobile_node(const std::string& id, const ip_prefix_list& prefs,
		...
		: node(id), _prefixes(prefs), _link_addrs(link_addrs), /*_lma_id(lma_id)*/_cmd_id(cmd_id),
	...
	//! Changed
	const std::string&       cmd_id() const         { return _cmd_id; }
